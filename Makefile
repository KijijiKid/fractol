...
$(NAME): $(OBJS) $(LIBFT_A)
	$(CC) $(LDFLAGS) $(OBJS) $(LIBFT_A) -Lmlx -lmlx -framework OpenGL -framework AppKit -o $(NAME)
	# Copy the dylib next to the binary
	cp mlx/libmlx.dylib .
	# Patch the binary to look for the dylib next to executable
	install_name_tool -change /Users/mandre/Desktop/42Projects/mlx/libmlx.dylib @executable_path/libmlx.dylib $(NAME)
